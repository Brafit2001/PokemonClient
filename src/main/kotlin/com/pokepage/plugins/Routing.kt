...
import com.pokepage.model.User
...
fun Application.configureRouting() {
    ...
    val homePaths = arrayOf("/home", "/home/search")
    routing {
        staticResources("/static", "static")
        homePaths.forEach { path -> get(path, home()) }

        get("/"){
            val user = User(0, "prueba", "", "")
            call.respond(ThymeleafContent("index", mapOf("user" to user)))
        }

        get("/login"){
            val user = User(0, "prueba", "", "")
            call.respond(ThymeleafContent("login", mapOf("user" to user)))
        }
...
private fun home(): suspend PipelineContext<Unit, ApplicationCall>.(Unit) -> Unit =
    {
        ...
        if (keyword == null || keyword == ""){
            ...
            call.respond(ThymeleafContent("home", mapOf("pokemonList" to pokemonList)))
